...
public interface TableRepository extends JpaRepository<RestaurantTable, Long> {
    ...
    @Query("select res from RestaurantTable res left join fetch res.waiter w where res.tableNum =?1 and w.username =?2")
    RestaurantTable findOneWithWaiter(int tableNum, String waiterUsername);
